...
(function($) {
  ...
  var switcherM = {
    ...
    makeSwitcher: function($input, switcherP, options) {
      ...
      data.$switcher = templates.$switcher.clone().attr({
      ...
      if (family = cat.group[inputName]) {
        ...
        cat.group[inputName] = family = {
          group: {},
          length: 1
        } //, enabled: 0, disabled: 0, switchable: false };
    ...
    setTemplateStyle: function(options, switcherP) {
      ...
      // Switcher
      templates.$switcher.css({
          'width': [options.width, 'px'].join(''),
          'line-height': [options.height, 'px'].join(''),
          'float': 'right' //ADDED
        })
      ...
      switcherP.cssValues.blobWidth = switcherP.cssValues.switcherHeight - options.blobOffset * 15;
      switcherP.cssValues.blobHeight = switcherP.cssValues.switcherHeight - options.blobOffset * 2;
      //Original Settings
      // switcherP.cssValues.blobWidth = switcherP.cssValues.blobHeight = switcherP.cssValues.switcherHeight - options.blobOffset * 2;

      templates.$blob.css({
        'width': switcherP.cssValues.blobWidth,
        'height': switcherP.cssValues.blobHeight
      });
      ...
      if (options.height < 20 || options.autoFontSize) templates.$toggler.css({
        'font-size': [options.height / 2, 'px'].join('')
      });
